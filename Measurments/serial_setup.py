...
def enc(s: str) -> bytes: return s.encode("utf-8", errors="ignore")
def dec(b: bytes) -> str:  return b.decode("utf-8", errors="ignore")
...
def serial_setup(port: str, address: str) -> Serial:
  ...
  s.write(enc("c[0,2,12]\n"))
  sleep(0.1)
  assert dec(s.read_until()) == "c[0,2,12]\n"
  return s